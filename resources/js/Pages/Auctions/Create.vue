...
const handleExtraImage = (e, index) => {
    ...
    const reader = new FileReader();
    reader.onload = (ev) => {
        extraPreviews.value[index] = ev.target.result;
    };
    reader.readAsDataURL(file);
};
...
const computedEndTimeUTC = computed(() => {
    ...
    const hours =
        durationOptions.find((d) => d.value === form.value.duration)?.hours ||
        24;
...
<template>
    ...
    <PulseBidLayout>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <Link
                href="/dashboard"
                class="inline-flex items-center gap-2 text-xs text-white/40 hover:text-primary transition-colors mb-8"
            >
                <svg
                    class="w-4 h-4"
                    fill="none"
            ...
            <!-- Header -->
            <div class="mb-10">
                <h1 class="font-display text-4xl sm:text-5xl font-bold">
                    Launch a New Auction
                </h1>
                <p class="mt-3 text-white/50 max-w-xl">
                    Present your masterpiece to the marketplace. Fill in the
                    details below to begin the live bidding process.
                </p>
            </div>
            ...
            <div class="grid lg:grid-cols-[380px_1fr] gap-8">
                ...
                <div class="space-y-4">
                    ...
                    >
                        <input
                            ...
                            accept="image/*"
                            class="hidden"
                            @change="(e) => handleMainImage(e.target.files[0])"
                        />
                        ...
                        <template v-if="imagePreview">
                            ...
                            />
                            <div
                                class="absolute inset-0 bg-surface-lowest/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity rounded-3xl"
                            >
                        ...
                        <template v-else>
                            <div
                                class="w-14 h-14 rounded-2xl bg-surface-bright flex items-center justify-center mb-4"
                            >
                                <svg
                            ...
                            <p
                                class="text-xs text-white/40 mt-1 text-center px-6"
                            >
                                Drag and drop high-resolution images or click to
                                browse
                            </p>
                            <p class="text-xs text-white/20 mt-3">
                                JPG, PNG, WEBP • MAX 10MB
                            </p>
                        </template>
                    </div>
                    <p v-if="errors.image" class="text-xs text-vivid">
                        {{ errors.image }}
                    </p>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        >
                            <input
                                ...
                                accept="image/*"
                                class="hidden"
                                @change="(e) => handleExtraImage(e, i - 1)"
                            />
                            <img
                                v-if="extraPreviews[i - 1]"
                                :src="extraPreviews[i - 1]"
                    ...
                    <!-- Pro tip -->
                    <div
                        class="bg-surface-container rounded-2xl p-4 border border-primary/20"
                    >
                        <p class="text-xs font-semibold text-primary mb-1">
                            Pro Tip
                        </p>
                        <p class="text-xs text-white/50 leading-relaxed">
                            Items with high-resolution, well-lit photographs
                            receive up to
                            <span class="text-primary font-semibold"
                                >45% higher engagement</span
                            >
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        <label
                            class="block text-xs font-semibold text-white/50 mb-2 tracking-wide"
                            >Auction Title</label
                        >
                        <input
                            v-model="form.title"
                        ...
                        />
                        <p v-if="errors.title" class="text-xs text-vivid mt-1">
                            {{ errors.title }}
                        </p>
                    </div>
                    ...
                    <div class="bg-surface-container rounded-3xl p-5">
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    <span
                                        class="absolute left-4 top-3 text-white/40 font-display font-bold"
                                        >$</span
                                    >
                                    <input
                    ...
                    <div class="bg-surface-container rounded-3xl p-5">
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                >
                                    <label
                                        class="block text-xs font-semibold text-white/50 tracking-wide"
                                ...
                                <div class="relative">
                                    ...
                                    />
                                    <svg
                                        class="absolute right-3 top-3 w-4 h-4 text-white/30 pointer-events-none"
                                        fill="none"
                                        viewBox="0 0 24 24"
                                        stroke="currentColor"
                                    >
                                ...
                                >
                                    <p
                                        v-if="relativeStartTime"
                                        class="text-[10px] text-primary font-medium"
                                    >
                                        {{ relativeStartTime }}
                                    </p>
                                    <p
                                        class="text-[10px] text-white/30 truncate ml-auto"
                                    >
